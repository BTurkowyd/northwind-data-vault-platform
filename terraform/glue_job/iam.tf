...
resource "aws_iam_policy" "glue_vpc_endpoint_policy" {
  name        = "GlueVpcEndpointPolicy"
  description = "Allows AWS Glue to validate VPC S3 endpoints"
...
resource "aws_iam_role_policy_attachment" "glue_vpc_endpoint_attachment" {
  role       = aws_iam_role.glue_role.name
  policy_arn = aws_iam_policy.glue_vpc_endpoint_policy.arn
}
